...
class Handler(SimpleHTTPRequestHandler):
    ...
    def _handle_cc_stats(self):
        """Sirve cc_stats_analisis.json enriquecido con totales de cc_financiero_v2.json."""
        ...
        fin_v2     = os.path.join(portal_dir, "data", "cc_financiero_v2.json")
        fin_v1     = os.path.join(portal_dir, "data", "cc_financiero.json")
        if not os.path.exists(stats_path):
            return self._send_json({"error": "No indexado aún."})
        try:
            with open(stats_path, encoding="utf-8") as f:
                data = json.load(f)
            # Preferir cc_financiero_v2.json (datos completos del indexador financiero)
            fin_path = fin_v2 if os.path.exists(fin_v2) else fin_v1
            if os.path.exists(fin_path):
                with open(fin_path, encoding="utf-8") as f:
                    cc_fin = json.load(f)
                if os.path.basename(fin_path) == "cc_financiero_v2.json":
                    ...
                    reportaron     = sum(1 for v in cc_fin.values()
                                        if (v.get("total_ingreso") or 0) > 0 or (v.get("total_gasto") or 0) > 0)
                else:
                    total_ingresos = sum((v.get("total_ingreso") or 0) for v in cc_fin.values())
                    total_gastos   = sum((v.get("total_gasto")   or 0) for v in cc_fin.values())
